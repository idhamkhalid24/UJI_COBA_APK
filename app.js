...
function leaveHomeCard(){
  if(!state.user)return '';
  const rows=leaveRows(), monthRows=leaveMonthRows(), pending=monthRows.filter(r=>leaveStatus(r)==='pending').length, latest=rows[0];
  const lock=leaveLockForDate();
  const unlock=pendingUnlockForLeave(lock?.id);
  const latestLine=lock?`Fitur terkunci: ${leavePeriodText(lock)}${unlock?' - menunggu admin buka fitur':''}`:(latest?`${leaveTypeLabel(latest.leaveType)} - ${leavePeriodText(latest)} - ${leaveStatusText(latest)}`:'Belum ada pengajuan bulan ini');
  const primary=lock?`<button class="btn primary" onclick="requestFeatureUnlock('${esc(lock.id)}')" ${unlock?'disabled':''}>${unlock?'Menunggu Admin':'Minta Buka'}</button>`:`<button class="btn success" onclick="openLeaveRequest()">Buat Izin</button>`;
  return `<div class="card leave-home-card"><div class="leave-home-main"><div class="leave-home-ico">I</div><div class="leave-home-copy"><div class="label">Izin Staff</div><div class="leave-home-title">${lock?'Fitur dikunci admin':(pending?`${pending} menunggu admin`:'Ajukan izin sakit / keperluan')}</div><div class="hint">${esc(latestLine)}</div></div></div><div class="leave-home-actions">${primary}<button class="btn" onclick="go('leave')">Riwayat</button></div></div>`;
}
...
async function submitLeaveRequest(){
  if(!state.user)return toast('Silakan login ulang');
  const u=key(state.user.username), leaveType=normalizeLeaveType($('leaveType')?.value), startDate=String($('leaveStart')?.value||'').slice(0,10), endDate=String($('leaveEnd')?.value||startDate).slice(0,10), reason=cleanLeaveText($('leaveReason')?.value,280);
...
function renderLeavePage(){
  ...
  const heroLabel=lock?'Minta Buka':'Buat Izin';
  page.innerHTML=`${top('Izin Staff',`${pending} menunggu - ${monthRows.length} bulan ini`)}${syncBar()}${lockCard}<div class="card leave-hero-card"><div><div class="label">Pengajuan Izin</div><div class="leave-hero-title">Sakit atau keperluan</div><div class="hint">Kirim izin ke admin dan cek status persetujuannya di sini.</div></div><button class="btn primary" onclick="${heroAction}">${heroLabel}</button></div>${body}`;
}
